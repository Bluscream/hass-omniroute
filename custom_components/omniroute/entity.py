...
from .const import CONF_MAX_TOKENS, CONF_TEMPERATURE, DOMAIN, LOGGER

MAX_TOOL_ITERATIONS = 10


def _adjust_schema(schema: dict[str, Any]) -> None:
    """Make the schema compatible with strict structured outputs."""
...
def _format_structured_output(
    ...
    """Format the schema for the structured output request."""
    result: JSONSchema = {
        "name": name,
        "strict": True,
    }
...
def _convert_content_to_chat_message(
    ...
    """Convert a chat log entry to the Chat Completions format."""
...
async def _transform_response(
    ...
    """Transform the API message into a ChatLog delta."""
...
class OmniRouteEntity(Entity):
    ...
    async def _async_handle_chat_log(
        ...
        model_args: dict[str, Any] = {
            ...
            "stream": False,
            "extra_headers": {"X-Title": "Home Assistant"},
        }

        if (max_tokens := self.subentry.data.get(CONF_MAX_TOKENS)) is not None:
            model_args["max_tokens"] = max_tokens
        if (temperature := self.subentry.data.get(CONF_TEMPERATURE)) is not None:
        ...
        for _iteration in range(MAX_TOOL_ITERATIONS):
            try:
                ...
                LOGGER.error("Error talking to OmniRoute: %s", err)
                raise HomeAssistantError("Error talking to OmniRoute") from err

            if not result.choices:
                LOGGER.error("OmniRoute returned empty choices")
                raise HomeAssistantError("OmniRoute returned an empty response")
